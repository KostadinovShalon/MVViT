...
from mmcv.cnn import build_activation_layer, Conv2d, Linear
from mmcv.cnn.bricks.transformer import build_positional_encoding, FFN
from mmcv.runner import force_fp32
from torch import nn

from mmdet.core import reduce_mean, build_assigner, build_sampler, multi_apply, bbox_cxcywh_to_xyxy, bbox_xyxy_to_cxcywh
from mmdet.models import HEADS, AnchorFreeHead, build_loss
from mmdet.models.utils import build_transformer


@HEADS.register_module()
class MVDETRHead(AnchorFreeHead):
    _version = 2
    ...
    def __init__(self,
                 ...
                 views=2,
                 single_view=False,
                 decoder_positional_encoding=dict(
        ...
        if class_weight is not None and (self.__class__ is MVDETRHead):
        ...
        self.transformer.single_view = single_view
        self.embed_dims = self.transformer.embed_dims
        assert 'num_feats' in positional_encoding
        num_feats = positional_encoding['num_feats']
        assert num_feats * 2 == self.embed_dims, 'embed_dims should' \
                                                 f' be exactly num_feats. Found {self.embed_dims}' \
                                                 f' and {num_feats}.'  # TODO: CHECK THIS
        self._init_layers()
    ...
    def _init_layers(self):
        ...
        if self.single_view:
            ...
            self.fc_cls = Linear(self.embed_dims, self.cls_out_channels * self.views)
        self.reg_ffn = FFN(
            self.embed_dims,
            self.embed_dims,
            self.num_reg_fcs,
        ...
        if self.single_view:
            ...
            self.fc_reg = Linear(self.embed_dims, 4 * self.views)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is MVDETRHead:
            convert_dict = {
    ...
    def forward_single(self, x, img_metas):
        ...
        masks = F.interpolate(
            masks.unsqueeze(1), size=(x.shape[1], x.shape[-2], x.shape[-1])).to(torch.bool).squeeze(1)  # bs v h w
        # position encoding
        encoder_pos_embed = self.positional_encoding(masks.view(-1, *masks.shape[-2:]))  # [bs*v, embed_dim, h, w]
        _masks = torch.cat([masks[:, m, ...] for m in range(views)], dim=-2)  # [bs, h*v, w]
        decoder_pos_embed = self.decoder_positional_encoding(masks)  # [bs, v, embed_dim, h, w]
        # outs_dec: [nb_dec, bs, num_query, embed_dim]  TODO: continuar aqui
        outs_dec, _ = self.transformer(x, masks, self.query_embedding.weight,
                                       (encoder_pos_embed, decoder_pos_embed))
        n, bs, nq, d = outs_dec.shape

        # If multiview, [nb_dec, b, num_query, embed_dim] else [nb_dec, b*views, num_query, embed_dim]
        # outs_dec = outs_dec.view(n, -1, nq, d)
        # If multiview, [nb_dec, b, num_query, C*views] else [nb_dec, b*views, num_query, C]
        all_cls_scores = self.fc_cls(outs_dec)
        if not self.transformer.single_view:
            all_cls_scores = torch.cat(
                [all_cls_scores[..., (self.cls_out_channels * view):(self.cls_out_channels * (view + 1))] for view in
                 range(views)], dim=1)
        # If multiview, [nb_dec, b, num_query, 4*v] else [nb_dec, b*views, num_query, 4]
        all_bbox_preds = self.fc_reg(self.activate(
            self.reg_ffn(outs_dec))).sigmoid()
        if not self.transformer.single_view:
            all_bbox_preds = torch.cat([all_bbox_preds[..., (4 * view):(4 * view + 4)] for view in range(views)], dim=1)
